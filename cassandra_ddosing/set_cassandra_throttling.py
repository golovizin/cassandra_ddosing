...
def get_data_mount_point(pod_name: str, namespace: str) -> str:
    ...
    for mount_point in ["/data/cassandra", "/var/lib/cassandra", "/"]:
        cmd = [
            ...
            "sh", "-c", f"test -d {mount_point} && echo {mount_point}"
        ]
...
def get_block_device(pod_name: str, namespace: str) -> str:
    ...
    cmd = [
        "kubectl", "exec", "-n", namespace, pod_name, "--",
        "sh", "-c", f"(df {mount_point} 2>/dev/null || df /) | tail -1 | awk '{{print $1}}'"
    ]
    ...
    if not device:
        ...
        print(f"  Попробуйте выполнить вручную: kubectl exec -n {namespace} {pod_name} -- df {mount_point}", file=sys.stderr)
        return ""
    
    if not device.startswith('/dev/'):
        print(f"✗ Устройство '{device}' не начинается с /dev/ для {pod_name}", file=sys.stderr)
        print(f"  Возможно, используется overlay или tmpfs. Попробуйте найти реальное блочное устройство.", file=sys.stderr)
        return ""
...
def get_device_major_minor(pod_name: str, namespace: str, mount_point: str) -> Tuple[str, str]:
    ...
    cmd = [
        "kubectl", "exec", "-n", namespace, pod_name, "--",
        "sh", "-c", f"grep '{mount_point}' /proc/self/mountinfo | head -1 | awk '{{print $3, $NF}}'"
    ]
    
    returncode, stdout, stderr = run_command(cmd)
    
    if returncode != 0:
        print(f"✗ Ошибка при получении информации об устройстве для {mount_point} в {pod_name}: {stderr}", file=sys.stderr)
        return "", ""
    ...
    cmd_base = [
        "minikube", "ssh", "--",
        f"stat -c '%t:%T' {device_path} 2>/dev/null || echo ''"
    ]
...
def get_container_id(pod_name: str, namespace: str) -> str:
    ...
    cmd = [
        "kubectl", "get", "pod", pod_name,
        "-n", namespace,
        "-o", "jsonpath={.status.containerStatuses[?(@.name=='cassandra')].containerID}"
    ]
...
@click.option(
    "--namespace",
    default="cassandra",
    help="Kubernetes namespace (по умолчанию: cassandra)"
)
@click.option(
    "--pod",
    help="Имя конкретного пода (если не указано, применяется ко всем Cassandra-подам)"
)
@click.option(
    "--dry-run",
    is_flag=True,
